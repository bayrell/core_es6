...
Core.UI.Render.CoreManager = class extends Runtime.ContextObject{
	/**
	...
	constructor(context){
		...
		introspection.each((info) => {
			var annotations = (Runtime.rtl.method(info.getClassName(), "filterAnnotations"))("", info);
	 ...
	 */
	initManager(){
		this.signal_in.addMethod(this.signalInModelChange.bind(this), (new Runtime.Vector()).push("Core.UI.Events.ModelChange"));
	}
	/**
	 * Destroy manager
	 */
	...
	setParentManager(parent_manager, ui){
		...
		*/
		...
		if (ui.bind != ""){
			annotations.push(new Core.UI.Annotations.BindModel((new Runtime.Map()).set("model", ui.bind)));
		}
		if (annotations != null && annotations.count() > 0){
			this.annotations_emitter = new Runtime.Emitter();
			for (var i = 0; i < annotations.count(); i++){
				var annotation = annotations.item(i);
				(Runtime.rtl.method(annotation.getClassName(), "addEmitter"))(this.parent_manager, this.annotations_emitter, ui, annotation);
			}
	...
	initAnnotation(info, annotation){
		...
		(Runtime.rtl.method(annotation.getClassName(), "initController"))(controller, this, annotation, field_name);
	}
	/**
	...
	setModel(model){
		if (this.model != model){
			...
			this.signal_out.dispatch(new Core.UI.Events.ModelChange((new Runtime.Map()).set("model", this.model)));
