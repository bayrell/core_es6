...
Core.FileSystem.Provider.FileSystemProvider = class extends Runtime.CoreStruct{
	/**
	...
	static createNode(context, fs, filepath, kind, options){
		...
		return (async_ctx_0) => {
			...
			if (async_jump_0 == "0"){
				...
				if (kind == Core.FileSystem.FileNode.KIND_FILE){
				}
				if (kind == Core.FileSystem.FileNode.KIND_FOLDER){
				}
				return async_ctx_0.resolve(res);
			}
			else if (async_jump_0 == "-1"){
	...
	static readBlock(context, fs, filepath, offset, count){
		...
		return (async_ctx_0) => {
			...
			if (async_jump_0 == "0"){
				...
				return async_ctx_0.resolve(new Core.FileSystem.FileIOResult((new Runtime.Map()).set("kind", Core.FileSystem.FileIOResult.KIND_READ_BINARY).set("name", filepath).set("offset", offset).set("bytes", bytes).set("count", count).set("size", size).set("eof", eof)));
			}
			else if (async_jump_0 == "-1"){
				return async_ctx_0.error( async_ctx_0.getError() )
			}
	...
	static writeBlock(context, fs, filepath, bytes, offset, count){
		return (async_ctx_0) => {
			...
			if (async_jump_0 == "0"){
				...
				return async_ctx_0.resolve(new Core.FileSystem.FileIOResult((new Runtime.Map()).set("kind", Core.FileSystem.FileIOResult.KIND_WRITE_BINARY).set("name", filepath).set("offset", offset).set("bytes", bytes).set("count", count).set("size", 0).set("eof", false)));
			}
			else if (async_jump_0 == "-1"){
				return async_ctx_0.error( async_ctx_0.getError() )
			}
	...
	assignObject(obj){
		if (obj instanceof Core.FileSystem.Provider.FileSystemProvider){
		}
