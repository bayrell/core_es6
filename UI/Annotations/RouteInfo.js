...
Core.UI.Annotations.RouteInfo = class extends Runtime.CoreStruct{
	/**
	 * Init struct data
	 */
	initData(){
		...
		if (matches){
			...
			params.each((name) => {
				uri_match = Runtime.re.replace("{"+Runtime.rtl.toString(name)+"}", "([^\\/]*?)", uri_match);
			});
		...
		this.assignValue("uri_match", "^"+Runtime.rtl.toString(uri_match)+"$");
	}
	/**
	 * Get params
	 * @return Map<string>
	...
	static getParams(matches, info){
		var __memorize_value = Runtime.rtl._memorizeValue("Core.UI.Annotations.RouteInfo::getParams", arguments);
		if (__memorize_value != Runtime.rtl._memorize_not_found) return __memorize_value;
		if (info.params == null){
			var __memorize_value = null;
			Runtime.rtl._memorizeSave("Core.UI.Annotations.RouteInfo::getParams", arguments, __memorize_value);
	...
	assignObject(obj){
		if (obj instanceof Core.UI.Annotations.RouteInfo){
	...
	assignValue(variable_name, value, sender){if(sender==undefined)sender=null;
		...
		else if (variable_name == "uri_match")this.__uri_match = Runtime.rtl.convert(value,"string","","");
		else if (variable_name == "layout_class")this.__layout_class = Runtime.rtl.convert(value,"string","","");
		else if (variable_name == "view_class")this.__view_class = Runtime.rtl.convert(value,"string","","");
		else if (variable_name == "render")this.__render = Runtime.rtl.convert(value,"bool",false,"");
		else if (variable_name == "params")this.__params = Runtime.rtl.convert(value,"Runtime.Collection",null,"string");
